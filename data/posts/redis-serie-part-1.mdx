---
title: 'Introdución a Redis parte 1'
date: '2021-10-15'
tags:
  - '💻 Development'
  - 'Redis'
...
Hace unos meses comencé a estudiar sobre la base de datos **open source** bajo la licencia **BSD** [redis](https://redis.io/). En la actualidad es utilizada como base de datos, cache y brokers de mensajería.
...
Redis trabaja siguiendo el concepto de ***in-memory dataset*** (almacenamiento en memoria) y en dependencia de nuestro caso de uso es posible configurar la opcion de persistir los datos en intervalos de tiempo en el disco duro.
...
- Instalación de utilidades y erramientas de configuración
...
Accedemos al archivo de configuración mediante la terminal de lineas de comando. Dentro de este archivo de configuración `redis.conf` es donde podremos definir el comportamiento de nuestra instacia de redis en artículos posteriores trabajaremos a fondo con este archivo.
...
Una vez creado nuestro fichero redis.services procesdemos a ejecutar y chequear el estado de nuestro servicio
...
En fin se que ha sido largo este tutorial pero no podemos dejar de pasar por alto nuestro comando de prueba para comprobar que el servicio se encuentre activo.
...
Llegado a este punto ya nos encontramos en condiciones para usar nuestra base de datos en futuros proyectos. Por lo que en próximos articúlos estaremos abordando temas como seguridad, clusters y desarrollo de aplicaciones. Espero que te sea de utilidad este contenido y nos vemos en el próximo capítulo.
