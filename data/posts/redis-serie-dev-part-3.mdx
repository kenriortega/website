...
Este `cmd` consiste en leer nuesto fichero de *datasets* `pizzas.csv` e insertar en redis los datos de interés para nuestro caso de uso. Lo curioso del método `IngestData`
...
```
> 📌 No todo es color de rosa, en este caso redis retorna cómo resultado un arreglo `[count, key,[key value,...],...]`, y llevarlo al formato de objeto json final fue un reto. Al final del tutorial te dejaré un enlace donde se implementa esa lógica pero con `javascript`. Y extrañé `javascript` sobre todo las funciones *map,reduce y filter*  en ese momento 🤪.
...
El tercer *endpoint* `/pizzas/stats` nos permite realizar una agregación de los restaurantes de pizzas, basada en la fecha que se le indique por parámetro. Con el objetivo de obtener la cantidad de restaurantes que se habían creado durante ese rango de fecha
...
A través de este tutorial, pudimos desarrollar una aplicación en go y redis, donde abordamos conceptos propios de la base de datos, cómo el uso de los *pipeline* y el trabajo con los *HSet* tipo de dato más avanzado.
