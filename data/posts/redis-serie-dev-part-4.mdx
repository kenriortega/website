...
Teniendo esta breve explicación espero que estes motivado en usar esta característica, como sí estuviesemos usando *apache kafka*. Para más detalles teóricos y ejemplos de los comandos utilizados en este tutorial puedes revisar el apartado de recursos.
...
El día de hoy teniendo como base este genial proyecto, vamos a realizar una reimplementación para el envio de eventos, provenientes  de los 🚍 de trasnporte municipal hacia nuestra instancia *redis*.
...
```go
    ...
    ...
```
En el fragemento de código anterior se encuentra implementado en go del comando *XADD* este recibe por parametros un `redis.XAddArgs` entre sus propiedades básicas y de interes para este turorial se destacan `Stream` contiene el nombre del tópico por donde se emitirán los eventos y `Value` hace mención a la data que se transmitirá.
...
- La fase inicial es la encargada de construir de forma dínamica nuestros tópicos y nombres de grupos de consumidores permitiendo crear nuestro `XGroupCreate`.
...
- Se abordaron conceptos referentes a sistemas de alto impacto relacionados con las colas de mensajes y comunicacion asíncrona en el envio de eventos.
- Se mostraron posibles casos de usos en la funcionalidad del productor y consumidor de eventos.
